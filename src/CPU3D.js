...
function CPU3D(threadCount, canvas, callback, filepath) {
	this.threadResponse = (function(event) {
		this.interpretMessage(event);
	}).bind(this);

	this.CPU3D(threadCount, canvas, callback, filepath);
}

///////////////////////////////////////////////////////////////////////////////
// Properites
///////////////////////////////////////////////////////////////////////////////
CPU3D.prototype.width;
CPU3D.prototype.height;

CPU3D.prototype.render;		//if threads are not being used, this is where the render object is stored.

CPU3D.prototype.callback;

CPU3D.prototype.threads;
CPU3D.prototype.threadCount;
CPU3D.prototype.threadResponseCount;

CPU3D.prototype.vertexBank;			//Stores a boolean for if an index is used. Mirrors what the render has.
CPU3D.prototype.textureBank;		//Stores a boolean for if an index is used. Mirrors what the render has.
CPU3D.prototype.shaderBank;			//Stores a boolean for if an index is used. Mirrors what the render has.

CPU3D.prototype.pixelBuffer_Uint8;	//stores the buffer that is returned piecemeal by the threads

CPU3D.prototype.messageQueue;

CPU3D.prototype.initialised; //flag set once initialise has been run. Stops any commands going through early.

CPU3D.prototype.canvas;
CPU3D.prototype.canvasContext;
CPU3D.prototype.imageData;

CPU3D.prototype.filepath; //stores the filepath to the library. May need to be edited if library is not in root and threads are used
...
// Constructor
///////////////////////////////////////////////////////////////////////////////
CPU3D.prototype.CPU3D = function(canvas, callback, threadCount, filepath) {	
...
// Methods
///////////////////////////////////////////////////////////////////////////////
CPU3D.prototype.createThreads = function(count) {
...
CPU3D.prototype.destroyThreads = function(count) {
...
CPU3D.prototype.sendMessage = function(messageData) {
	for(var i = 0; i < this.threadCount; i++) {
...
CPU3D.prototype.sendMessages = function() {
	if (this.CONT_USING_THREADS) {
		for(var i = 0; i < this.threadCount; i++) {
			var thread = this.threads[i];
...
CPU3D.prototype.addMessageToQueue = function(messageData) {
	this.messageQueue.push(messageData);
	return;
}

CPU3D.prototype.interpretMessage = function(event) {
...
/////////////////////////////////////////////////////////////////////////////////
CPU3D.prototype.getBufferProcessing = function(pixels, offset) {
...
CPU3D.prototype.getBufferCanvas = function(pixels) {
...
CPU3D.prototype.getBufferCanvasThreaded = function(pixels, offset) {
	//put the pixel map in to the imageData object
	if (this.CONT_USING_ARRAY_SET) {
...
CPU3D.prototype.getBufferCallback = function(pixels) {
	this.callback(pixels);
	return;
}

CPU3D.prototype.getBufferCallbackThreaded = function(pixels, offset) {
	if (this.CONT_USING_ARRAY_SET) {
...
CPU3D.prototype.getBufferCanvasCallback = function(pixels) {
	//put the pixel map in to the imageData object
	if (this.CONT_USING_ARRAY_SET) {
		this.imageData.data.set(pixels);
	} else {
...
CPU3D.prototype.getBufferCanvasCallbackThreaded = function(pixels, offset) {
	//put the pixel map in to the imageData object
	if (this.CONT_USING_ARRAY_SET) {
		this.imageData.data.set(pixels, offset);
	} else {
...
CPU3D.prototype.setCanvas = function(canvas) {
	if (typeof canvas == "object") {
		this.canvas = canvas;
		this.canvasContext = this.canvas.getContext("2d");
		this.imageData = this.canvasContext.createImageData(this.canvas.width, this.canvas.height);
...
CPU3D.prototype.initialise = function(width, height) {
...
CPU3D.prototype.resetScreen = function(width, height) {
	//make sure our input is correct.
	width = width << 0;
	height = height << 0;
	if (width < 1) {
...
CPU3D.prototype.draw = function(vboID, start, length) {	
	if (this.CONT_USING_THREADS) {
...
*/
CPU3D.prototype.getBuffer = function() {
	var buffer = 1;
...
CPU3D.prototype.clearBuffer = function(buffer) {
	if (this.initialised == false) {
		console.log("CPU3D Error: CPU3DCore has not been initialised yet. Use setTimeout or setInterval to delay draw commands after initialing.");
		return;
	}
...
CPU3D.prototype.clearColor = function(red, green, blue, alpha) {
	if (this.initialised == false) {
		console.log("CPU3D Error: CPU3DCore has not been initialised yet. Use setTimeout or setInterval to delay draw commands after initialing.");
		return;
	}
...
CPU3D.prototype.getVBOId = function() {
	if (this.initialised == false) {
		console.log("CPU3D Error: CPU3DCore has not been initialised yet. Use setTimeout or setInterval to delay draw commands after initialing.");
		return;
	}
...
CPU3D.prototype.loadVBO = function(vbo, data) {
	if (this.initialised == false) {
		console.log("CPU3D Error: CPU3DCore has not been initialised yet. Use setTimeout or setInterval to delay draw commands after initialing.");
		return;
	}
...
CPU3D.prototype.clearVBO = function(vbo) {
	if (this.initialised == false) {
		console.log("CPU3D Error: CPU3DCore has not been initialised yet. Use setTimeout or setInterval to delay draw commands after initialing.");
		return;
	}
...
CPU3D.prototype.getTBOId = function() {
	if (this.initialised == false) {
		console.log("CPU3D Error: CPU3DCore has not been initialised yet. Use setTimeout or setInterval to delay draw commands after initialing.");
		return;
	}
...
CPU3D.prototype.loadTBO = function(tbo, data, width, height) {
	if (this.initialised == false) {
		console.log("CPU3D Error: CPU3DCore has not been initialised yet. Use setTimeout or setInterval to delay draw commands after initialing.");
		return;
	}
...
CPU3D.prototype.bindTBO = function(tbo, textureSlot) {
...
CPU3D.prototype.clearTBO = function(tbo) {
	if (this.initialised == false) {
		console.log("CPU3D Error: CPU3DCore has not been initialised yet. Use setTimeout or setInterval to delay draw commands after initialing.");
		return;
	}
...
CPU3D.prototype.getSBOId = function() {
	if (this.initialised == false) {
		console.log("CPU3D Error: CPU3DCore has not been initialised yet. Use setTimeout or setInterval to delay draw commands after initialing.");
		return;
	}
...
CPU3D.prototype.loadSBO = function(sbo) {
	if (this.initialised == false) {
		console.log("CPU3D Error: CPU3DCore has not been initialised yet. Use setTimeout or setInterval to delay draw commands after initialing.");
		return;
	}
...
CPU3D.prototype.bindSBO = function(sbo) {
	if (this.initialised == false) {
		console.log("CPU3D Error: CPU3DCore has not been initialised yet. Use setTimeout or setInterval to delay draw commands after initialing.");
		return;
	}
...
CPU3D.prototype.clearSBO = function(sbo) {
	if (this.initialised == false) {
		console.log("CPU3D Error: CPU3DCore has not been initialised yet. Use setTimeout or setInterval to delay draw commands after initialing.");
		return;
	}
...
CPU3D.prototype.assignSBOVertexShader = function(sbo, filepath, functionName) {
...
CPU3D.prototype.assignSBOPixelShader = function(sbo, filepath, functionName) {
	if (this.initialised == false) {
		console.log("CPU3D Error: CPU3DCore has not been initialised yet. Use setTimeout or setInterval to delay draw commands after initialing.");
		return;
	}
...
CPU3D.prototype.addSBOVertexAttribute = function(sbo, shader, type, length) {
	if (this.initialised == false) {
		console.log("CPU3D Error: CPU3DCore has not been initialised yet. Use setTimeout or setInterval to delay draw commands after initialing.");
		return;
	}
...
CPU3D.prototype.setShaderVariable = function(name, value) {
	if (this.initialised == false) {
		console.log("CPU3D Error: CPU3DCore has not been initialised yet. Use setTimeout or setInterval to delay draw commands after initialing.");
		return;
	}
...
CPU3D.prototype.setProperty = function(name, value) {
	if (this.initialised == false) {
		console.log("CPU3D Error: CPU3DCore has not been initialised yet. Use setTimeout or setInterval to delay draw commands after initialing.");
		return false;
	}
